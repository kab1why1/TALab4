...
function updateDisplay(container, stackItems) {
  ...
  stackItems.forEach(student => {
    ...
    // Відображаємо ім'я, бал та метод додавання студента
    li.textContent = `${student.name} - ${student.score} (${student.method})`;
...
// Створення об’єкта CombinedStack для кожної групи із попереднім заповненням 3-ма студентами
const groupsData = [];
document.querySelectorAll('.list-container').forEach((group, index) => {
  ...
  // Попереднє заповнення: додаємо 3 випадкових студентів (за замовчуванням використовується режим "Масив")
  for (let i = 1; i <= 3; i++) {
    const randomScore = getRandomScore(68, 93);
    stack.pushArray({ name: `Student ${i}`, score: randomScore, method: "Масив" });
  }
  ...
  addBtn.addEventListener('click', () => {
    ...
    if (mode === "array") {
      stack.pushArray({ name, score, method: "Масив" });
    } else {
      stack.pushPointer({ name, score, method: "Покажчик" });
    }
    updateDisplay(group, stack.getItems());
    nameInput.value = "";
    scoreInput.value = "";
  ...
  // При зміні режиму оновлюємо відображення (дані залишаються спільними)
...
// Єдиний аналіз успішності груп (незалежно від методу додавання/видалення)
document.getElementById('analyze-btn').addEventListener('click', () => {
  ...
  for (let i = 0; i < processed.length - 2; i++) {
    ...
    analysisText += `<p>Групи ${i+1}-${i+3}: [${three.map(num => num.toFixed(2)).join(', ')}] | Середнє: ${avgThree.toFixed(2)} | Тенденція: ${trend}</p>`;
  }
